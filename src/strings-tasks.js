...
function formatTime(minutes, seconds) {
  const updMin = String(minutes).padStart(2, '0');
  const updSec = String(seconds).padStart(2, '0');

  return updMin.concat(':', updSec);
}
...
function isPalindrome(str) {
  const strPalindrom = str.toLowerCase().replace(/[^a-z0-9]/g, '');
  return strPalindrom === strPalindrom.split('').reverse().join('');
}
...
function getStringFromTemplate(firstName, lastName) {
  const hi = 'Hello, ';
  return hi + firstName.concat(' ', lastName, '!');
}
